...
class SAPIntegration:
    def __init__(self):
        self.base_url = os.environ.get('SAP_B1_URL', 'https://sap-b1-server:50000/b1s/v1/')
        self.company_db = os.environ.get('SAP_B1_COMPANY_DB', 'SBODEMOUS')
        self.username = os.environ.get('SAP_B1_USERNAME', 'manager')
        self.password = os.environ.get('SAP_B1_PASSWORD', 'manager')
        ...
        try:
            ...
            for line in grpo.grpo_lines:
                ...
                grpo_line = {
                    ...
                    'BaseLine': po_line.line_number - 1  # SAP uses 0-based indexing
                }
                ...
                if line.batch_number:
                    grpo_line['BatchNumbers'] = [{
                        ...
                        'Quantity': float(line.received_quantity),
                        'ExpiryDate': line.expiry_date.isoformat() if line.expiry_date else None
                    }]
            ...
            response = requests.post(
                f"{self.base_url}PurchaseReceipts",
                json=grpo_data,
                cookies=cookies,
                verify=False,
                timeout=60
            ...
            if response.status_code == 201:
                ...
                logging.info(f"GRPO {grpo.grn_number} posted to SAP B1 as DocEntry {doc_entry}")
                return doc_entry
            else:
                logging.error(f"SAP B1 GRPO posting failed: {response.status_code} - {response.text}")
                return None
    ...
    def get_purchase_orders(self, branch_id=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(f"SAP B1 PO retrieval failed: {response.status_code}")
                return []
            ...
            for sap_po in sap_pos:
                ...
                if not existing_po:
                    ...
                    po = PurchaseOrder(
                        ...
                        po_date=datetime.strptime(sap_po['DocDate'], '%Y-%m-%d').date(),
                        delivery_date=datetime.strptime(sap_po['DocDueDate'], '%Y-%m-%d').date(),
                    ...
                    for line in sap_po['DocumentLines']:
                        po_line = PurchaseOrderLine(
                            po_id=po.id,
                            line_number=line['LineNum'] + 1,  # Convert to 1-based
